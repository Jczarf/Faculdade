...
def detect(path: str | None = None, *, all_files: bool = False) -> dict:
    ...
    if all_files: cmd += ["--all-files", "--exclude-files", EXCLUDE_RE.pattern]
    elif path: cmd.append(path)
...
def main() -> int:
    ...
    for raw in sorted(set(run("git", "log", "--all", "--format=", "--name-only").splitlines())):
        path = raw.strip()
        if path and not path.endswith(".env.example") and SENSITIVE_PATH_RE.search(path): problems.append(f"histórico: nome de arquivo sensível alcançável: {path}")
